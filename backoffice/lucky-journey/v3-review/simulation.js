...
(function(root,factory){if(typeof module==='object'&&module.exports)module.exports=factory(require('./engine.js'),require('./config.js'));else root.LJSimulation=factory(root.LuckyJourneyEngine,root.LJConfig);})(typeof window==='undefined'?globalThis:window,function(E,C){
  ...
  class Simulation {
    ...
    empty(){return {status:'unjoined',k:0,progress:0,progressUnits:0,collectibles:{coin:0,gem:0,star:0},tickets:0,rewards:{cash:0,credit:0,point:0},granted:{free:0,task:0,assist:0},tasks:[],records:[],reserve:0,claimPending:false,claimed:false};}
    ...
    join(){this.expire();if(this.play.status==='active')return {ok:false,message:'已参加本局。'};
      ...
      if(this.available()<cost.hardReserve)return {ok:false,message:'本期名额已满。'};
    ...
    spin(requestId){if(this.requestCache.has(requestId))return {ok:true,replayed:true,record:this.requestCache.get(requestId),message:'已恢复原开奖结果。'};
      ...
      const isManualClaim = completed && cfg.basic?.claimMode === 'manual';
      if(finish&&this.payout==='pending'){this.pending.push(record);this.reserved=money(this.reserved+actual);}
      else if(finish&&!isManualClaim){this.post(record);p.claimed=true;}
      else if(isManualClaim){p.claimPending=true;record.claimPending=true;}
      if(completed){p.status='completed';p.tickets=0;this.reserved=Math.max(0,money(this.reserved-p.reserve));p.reserve=0;}
      p.records.push(record);this.requestCache.set(requestId,record);
      this.log('第 '+k+' 转：'+prize.label+(finish?'；转满解锁 '+finish:'')+(isManualClaim?'（待手动领取）':finish&&this.payout==='pending'?'，派奖处理中。':'。'));
    ...
    claimPrize(){
      if(!this.play.claimPending)return {ok:false,message:'暂无可领取的奖金。'};
      const record=this.play.records.find(r=>r.completed);
      if(!record)return {ok:false,message:'未找到完成记录。'};
      this.post(record);
      this.play.claimPending=false;
      this.play.claimed=true;
      record.claimPending=false;
      this.log('手动领取转满奖金 '+record.finish.toFixed(2)+' 现金入账。');
      return {ok:true,message:'已成功领取奖金 '+record.finish.toFixed(2)+' 现金！'};
    }
