(() => {
  ...
  function render(){
    ...
    const main=$('#mainAction');main.textContent=busy?'开奖中…':phoneFrozen?'恢复连接':joining&&joinBlock?joinBlock:p.status==='unjoined'?'参加活动':p.status==='active'?(p.tickets?'转一次':'获取抽奖次数'):(config.basic.repeat&&sim.campaignOpen?'再次参加':label[p.status]);
    ...
    $('#friendRow').hidden=!c.sources.assist.enabled;$('#friendDesc').textContent='直接新注册好友'+(c.sources.assist.depositRequired?'累计成功充值满 '+money(c.sources.assist.minDeposit)+' 后':'')+'助力 · 每人 '+c.sources.assist.ticketsPerFriend+' 次';$('#friendBtn').disabled=!!disabled||p.granted.assist>=c.sources.assist.cap;
    ...
    $('#sourceLabel').textContent='已载入后台设置 · 转满 '+config.targetSpins+' 次';renderChecks();
  }
  ...
  function scenario(name){reset(config,false);if(name==='fresh')return;
  ...
  $('#mainAction').onclick=()=>{if(phoneFrozen){phoneFrozen=null;handle(sim.spin(request));return;}if(sim.play.status==='unjoined'||sim.play.status!=='active')handle(sim.join());else if(!sim.play.tickets){$('#ticketSources').scrollIntoView({behavior:$('#reduceMotion').checked?'auto':'smooth',block:'start'});$('#ticketSources').focus({preventScroll:true});}else spin();};
