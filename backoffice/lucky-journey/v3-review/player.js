(() => {
  ...
  $('#publishBtn').onclick=()=>{const status=renderChecks();if(!status.canPublish)return;if(status.yellow.length&&!confirm('存在 '+status.yellow.length+' 项提示，确认模拟发布？'))return;try{localStorage.setItem(C.key+'-release',JSON.stringify({config,forecast:{dailyVisitProb:Number($('#visitProb').value)/100,taskCompletionProb:Number($('#taskProb').value)/100,assistLambda:Number($('#friendLambda').value)},checkedAt:new Date().toISOString(),status:'prototype'}));$('#publishResult').textContent='模拟发布完成 · 未写入线上系统';}catch(e){$('#publishResult').textContent='本机保存失败。';}};
