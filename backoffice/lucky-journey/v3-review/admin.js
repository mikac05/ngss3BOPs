(() => {
  ...
  function field(label,path,{min=0,max=1000000,step=1,unit='',hint='',type='number',options=null,subHint='' }={}){
    const val=C.get(cfg,path),id='f-'+path.replaceAll('.','-');
    let control=options?`<select id="${id}" data-path="${path}">${options.map(([v,t])=>`<option value="${v}" ${v===val?'selected':''}>${t}</option>`).join('')}</select>`:`<input id="${id}" data-path="${path}" type="${type}" value="${esc(val)}" ${type==='number'?`min="${min}" max="${max}" step="${step}"`:''}>`;
  ...
  function build(){
    ...
    const wheelImgsHtml = `<div class="wheel-images-box"><div class="subhead"><h3>转盘奖项图片编辑</h3><span class="muted">自定义上传图片替换默认图示，支持 PNG/JPG/SVG/WebP</span></div><div class="wheel-images-grid">${['thanks','coin','gem','star'].map(k=>`<div class="wheel-image-card"><div class="wheel-image-thumb"><img id="wheel-thumb-${k}" src="${esc(cfg.presentation.wheelImages[k])}" alt="${imgLabels[k]}"></div><div class="wheel-image-meta"><strong>${imgLabels[k]}</strong><div class="wheel-image-actions"><label class="button button-sm upload-label">上传图片<input type="file" accept="image/*" data-upload-wheel="${k}" hidden></label><button type="button" class="button button-sm text-btn" data-reset-wheel="${k}">恢复默认</button></div></div></div>`).join('')}</div></div>`;
  ...
  function render(){
    ...
    $('#wagerField').hidden=cfg.basic.wallet!=='cash';$('[data-path="basic.name"]').readOnly=cfg.basic.nameMode==='system';$('#nameMode').value=cfg.basic.nameMode;
    ...
    ['thanks','coin','gem','star'].forEach(k=>{const el=$('#wheel-thumb-'+k);if(el&&cfg.presentation?.wheelImages?.[k])el.src=cfg.presentation.wheelImages[k];});
