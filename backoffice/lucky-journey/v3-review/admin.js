(() => {
  ...
  function build(){
    const floatIconsHtml = `<div class="field float-icon-field"><label>浮窗图标${tip('玩家端活动浮窗入口使用的图标。')}</label><div class="float-icon-list" role="radiogroup" aria-label="浮窗图标">${C.DEFAULT_FLOAT_ICONS.map(item=>`<button type="button" class="float-icon-tile" data-float-icon="${item.id}" role="radio" aria-checked="${cfg.presentation.floatIcon===item.id}" aria-label="${item.label}" title="${item.label}"><img src="${item.src}" alt=""><span class="tile-check" aria-hidden="true">✓</span></button>`).join('')}</div></div>`;
    ...
    $('#basic .panel-body').insertAdjacentHTML('beforeend',floatIconsHtml);
  ...
  function render(){
    ...
    document.querySelectorAll('[data-float-icon]').forEach(el=>{
      const selected=el.dataset.floatIcon===cfg.presentation.floatIcon;
  ...
  $('#sections').addEventListener('click',e=>{
    ...
    const icon=e.target.closest('[data-float-icon]');if(icon&&C.DEFAULT_FLOAT_ICONS.some(item=>item.id===icon.dataset.floatIcon)){cfg.presentation.floatIcon=icon.dataset.floatIcon;dirty=true;render();}
  ...
  $('#sections').addEventListener('keydown',e=>{
    if(!['ArrowLeft','ArrowRight'].includes(e.key)||!e.target.matches('[data-float-icon]'))return;
    const tiles=[...document.querySelectorAll('[data-float-icon]')],index=tiles.indexOf(e.target);
    const next=tiles[(index+(e.key==='ArrowRight'?1:-1)+tiles.length)%tiles.length];
